            ...
            <Box sx={{ color: 'text.primary', wordBreak: 'break-word', lineHeight: 1.6, whiteSpace: 'pre-wrap', '& > *:first-of-type': { mt: 0 }, '& > *:last-of-type': { mb: 0 } }}>
              <ReactMarkdown
                ...
                components={{
                  ...
                  blockquote: ({ children }) => (
                    <Box component="blockquote" sx={{ borderLeft: `3px solid ${warmPrimary}`, pl: 1.5, ml: 0, color: '#8B7355', fontStyle: 'italic', mb: 1 }}>{children}</Box>
                  ),
                  a: ({ children, href }) => (
                    <Box component="a" href={href} target="_blank" rel="noopener noreferrer" sx={{ color: warmPrimary, textDecoration: 'underline', '&:hover': { color: '#D4835A' } }}>{children}</Box>
                  ),
                  ...
                  th: ({ children }) => (
                    <Box component="th" sx={{ border: '1px solid #F5E6D3', p: 0.75, textAlign: 'left', fontWeight: 700, color: warmText }}>{children}</Box>
                  ),
                  td: ({ children }) => (
